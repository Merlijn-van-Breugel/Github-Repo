...
plot.homals <-
           ...
           ...)
  {
    ...
    if (plot.type == "biplot") {
      ...
      xycoorload <- t(sapply(x$loadings, function(xy) xy[1,c(pd1,pd2)]))
      xycoor <- rbind(xycoorload, x$objscores[,c(pd1,pd2)]*m^.5)
      ...
      if (show.points[1]) plot(x$objscores[,c(pd1,pd2)]*m^.5, type = "p", pch = 1, cex = 0.1, col = "#7777FF", main = main1, xlab = xlab, ylab = ylab, xlim = xlim, ylim = ylim, ...) 
      if (show.labels[1]) text(x$objscores[,c(pd1,pd2)]*m^.5, labels = rownames(x$dframe), col = 1)
